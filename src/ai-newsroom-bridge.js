...
export async function newsroomFetch(req,env){
 ...
 if(req.method==='POST'&&u.pathname==='/ai-newsroom/claim'){const b=await req.json();const r=await env.DB.prepare("UPDATE ai_newsroom_queue SET status='processing',claimed_at=CURRENT_TIMESTAMP WHERE id=? AND status='pending'").bind(Number(b.id)).run();return json({status:'ok',claimed:Boolean(r.meta?.changes)})}
 if(req.method==='POST'&&u.pathname==='/ai-newsroom/result'){
  const b=await req.json();if(!b.id||!b.result)return json({status:'error',error:'id_and_result_required'},400);
  const decision=String(b.result.publish_decision||'review');const status=['publish','review','reject'].includes(decision)?decision:'review';
  ...
  const row=await env.DB.prepare('SELECT * FROM ai_newsroom_queue WHERE id=?').bind(Number(b.id)).first();
  let editor_sent=false,editor_error=null;
  if(row&&env.V22_EDITOR_TELEGRAM_ENABLED==='true')try{await sendEditorCard(env,row,b.result);editor_sent=true;await env.DB.prepare("UPDATE ai_newsroom_queue SET status='editor_pending' WHERE id=?").bind(Number(b.id)).run()}catch(e){editor_error=String(e?.message||e).slice(0,180)}
