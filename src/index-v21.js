...
async function notify(env,i,decision){
 ...
 if(imageOk){
   try{
     const r=await fetch(i.image,{headers:{'User-Agent':'Mozilla/5.0'}});
     if(r.ok){f.append('photo',new Blob([new Uint8Array(await r.arrayBuffer())],{type:r.headers.get('content-type')||'image/jpeg'}),'90gundem.jpg');endpoint='sendPhoto'}
   }catch{}
 }
 if(endpoint==='sendMessage') f.append('text',(prefix+x).slice(0,4000));
 const r=await fetch(`https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/${endpoint}`,{method:'POST',body:f});
...
async function growthRun(env,manual=false){
 ...
 const started=new Date().toISOString();await setState(env,'growth_heartbeat',started);
 const items=await fetchPusholder();
 const cursor=Number(await state(env,'growth_last_id')||0),latest=items.reduce((m,x)=>Math.max(m,Number(x.pusholder_id)||0),0);
 if(!cursor){if(latest)await setState(env,'growth_last_id',latest);return {status:'ok',version:'growth-v21',mode:'cursor_initialized',last_id:latest}}
 ...
 const recent=(rows.results||[]).map(x=>x.title);let published=0,skipped=0,failed=0,last=cursor,errors=[];
 for(const p of pending){const id=Number(p.pusholder_id);try{
   ...
   await notify(env,i,decision);published++;recent.push(i.title);
   await env.DB.prepare("INSERT OR IGNORE INTO speed_queue(fingerprint,title,status) VALUES(?,?,'growth_ready')").bind(fp,i.title).run();last=Math.max(last,id);
 }catch(e){failed++;errors.push(String(e?.message||e).slice(0,140));break}}
 if(last>cursor)await setState(env,'growth_last_id',last);
 const out={status:failed?'partial':'ok',version:'growth-v21',manual,at:started,scanned:items.length,new_items:pending.length,published,skipped,failed,cursor_before:cursor,cursor_after:last,errors};
...
export default {
 async fetch(req,env,ctx){const u=new URL(req.url);
   ...
   if(u.pathname==='/growth-status'){const raw=await state(env,'growth_status');return Response.json({status:raw?'ok':'waiting',version:'growth-v21',heartbeat:await state(env,'growth_heartbeat'),last_id:await state(env,'growth_last_id'),last_run:raw?JSON.parse(raw):null})}
   if(u.pathname==='/health'){const r=await live.fetch(req,env,ctx);let d={};try{d=await r.clone().json()}catch{}return Response.json({...d,growth_version:'v21',growth_engine:true,growth_min_score:Number(env.GROWTH_MIN_SCORE||50),growth_image_passthrough:true})}
